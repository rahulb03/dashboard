...
import { revokePermission, fetchUsersWithPermissions } from '@/redux/permissions/permissionThunks';

const revokePermissionSchema = z.object({
  userId: z.string().min(1, { message: 'Please select a user' }),
  permissionIds: z.array(z.string()).min(1, { message: 'Please select at least one permission to revoke' }),
...
export default function RevokePermissionPage() {
  ...
  const form = useForm({
    resolver: zodResolver(revokePermissionSchema),
    defaultValues: {
      userId: '',
      permissionIds: [],
      reason: ''
  ...
  // Only show permissions that the selected user currently has
  const userPermissions = React.useMemo(() => {
    if (!selectedUser) return [];
    return (selectedUser.permissions || []).map(permission => ({
      ...permission,
      categoryName: permission.category || 'Other'
    }));
  }, [selectedUser]);
  ...
  const categories = React.useMemo(() => {
    ...
    const uniqueCategories = [...new Set(userPermissions.map(p => p.categoryName))];
  ...
  const filteredPermissions = React.useMemo(() => {
    ...
    return filtered;
  }, [userPermissions, selectedCategory, searchTerm]);
  
  const selectedPermissions = userPermissions.filter(p => 
  ...
  const handleSubmit = async (data) => {
    startTransition(async () => {
      try {
        // Revoke multiple permissions sequentially
        for (const permissionId of data.permissionIds) {
          ...
          await dispatch(revokePermission(formattedData)).unwrap();
        }
        ...
        toast.success(
          `${permissionCount} permission${permissionCount > 1 ? 's' : ''} revoked successfully!`
        );
  ...
  const formatExpiryDate = (expiryDate) => {
    ...
    return {
      ...
      isExpired,
      daysLeft: isExpired ? 0 : Math.ceil((date - now) / (1000 * 60 * 60 * 24))
    };
  };
  ...
  // Filter users to only show those with permissions
  const usersWithPermissions = users.filter(user => user.permissions && user.permissions.length > 0);
        ...
        return (
          <div className="flex items-center space-x-2">
            <div className={`w-4 h-4 border-2 rounded transition-all ${
              isSelected ? 'bg-destructive border-destructive' : 'border-muted-foreground/30'
            }`}>
              {isSelected && <CheckCircle className="h-3 w-3 text-destructive-foreground" />}
            </div>
          </div>
        );
      }
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-muted-foreground">
                {user.permissions?.length || 0} permissions to revoke
              </div>
            </div>
          </div>
        );
  ...
  const steps = [
    { number: 1, title: 'Select User', description: 'Choose the user to revoke permissions from' },
    { number: 2, title: 'Choose Permissions', description: 'Select the permissions to revoke' },
    { number: 3, title: 'Confirm & Submit', description: 'Provide reason and confirm action' }
  ];
  ...
  return (
    <div className="flex flex-col space-y-6 p-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Revoke Permissions</h1>
        <p className="text-muted-foreground text-sm">Remove one or more permissions from a user's access</p>
      </div>
          ...
          {currentStep === 1 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Choose the user to revoke permissions from</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {usersWithPermissions.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <AlertCircle className="h-16 w-16 text-orange-400 mb-4" />
                    <h3 className="text-lg font-semibold mb-2">No Users with Permissions</h3>
                    <p className="text-muted-foreground">There are no users with permissions to revoke</p>
                  </div>
                ) : (
                  <FormField
          ...
          {currentStep === 2 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select one or more permissions to revoke from <strong>{selectedUser?.name}</strong>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {userPermissions.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <AlertCircle className="h-16 w-16 text-orange-400 mb-4" />
                    <h3 className="text-lg font-semibold mb-2">No Permissions to Revoke</h3>
                    <p className="text-muted-foreground">{selectedUser?.name} has no permissions to revoke</p>
                    <Button 
                      variant="outline" 
                              ...
                              {filteredPermissions.length === 0 ? (
                                ...
                                <>
                                  <div className="text-sm text-muted-foreground mb-4">
                                    {filteredPermissions.length} permission{filteredPermissions.length !== 1 ? 's' : ''} available to revoke
                                  ...
                                  <div className="space-y-2">
                                    {filteredPermissions.map((permission) => {
                                      ...
                                      const expiryInfo = formatExpiryDate(permission.expiresAt);
                                      return (
                                        <div
                                          key={permission.id}
                                          onClick={() => {
                                        ...
                                        >
                                          <div className="flex items-start space-x-3">
                                            ...
                                            `}>
                                              {isSelected && (
                                                <CheckCircle className="h-3 w-3 text-destructive-foreground" />
                                              )}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                              ...
                                              {expiryInfo && (
                                                <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                                                  ...
                                                  {expiryInfo.isExpired ? (
                                                    <span className="text-destructive font-medium">Expired {expiryInfo.formatted}</span>
                                                  ) : (
              ...
              <CardContent className="space-y-6">
                ...
                <div className="bg-destructive/10 border border-destructive/20 p-4 rounded-lg">
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="flex-1">
                      <div className="font-medium text-destructive text-sm mb-2">
                        You are about to revoke {selectedPermissions.length} permission{selectedPermissions.length > 1 ? 's' : ''}:
                      </div>
                      <p className="text-xs text-destructive">
                        This action cannot be undone. The user will immediately lose access to the selected permissions.
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        <span className="text-sm text-muted-foreground">
                          Permission{selectedPermissions.length > 1 ? 's' : ''} to revoke:
                        </span>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-base font-semibold">Reason for Revocation *</FormLabel>
                      <FormControl>
                        <Textarea 
                          placeholder="Explain why these permissions are being revoked..."
                          disabled={loading}
                          rows={4}
