...
export const createSalaryThunk = createAsyncThunk(
  ...
  async (salaryData, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(API_ENDPOINTS.SALARY.CREATE, salaryData);
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to create salary configuration';
...
export const fetchSalariesThunk = createAsyncThunk(
  ...
  async ({ forceRefresh = false } = {}, { rejectWithValue }) => {
    try {
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to fetch salary configurations';
...
export const fetchSalaryByIdThunk = createAsyncThunk(
  ...
  async ({ salaryId, forceRefresh = false }, { rejectWithValue }) => {
    try {
      ...
      const response = await axiosInstance.get(API_ENDPOINTS.SALARY.GET_ONE(salaryId));
      const data = response.data.data;
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to fetch salary configuration';
      return rejectWithValue(message);
    }
  }
);
...
export const updateSalaryThunk = createAsyncThunk(
  ...
  async ({ salaryId, salaryData }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.put(API_ENDPOINTS.SALARY.UPDATE(salaryId), salaryData);
      ...
      dataCache.optimisticUpdate('salaries', (cachedSalaries) => {
        if (Array.isArray(cachedSalaries)) {
          return cachedSalaries.map(salary => 
            salary.id === parseInt(salaryId) ? { ...salary, ...updatedSalary } : salary
          );
        }
        return cachedSalaries;
      });
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to update salary configuration';
      const validationErrors = error?.response?.data?.data || {};
      return rejectWithValue({ message, validationErrors });
    }
  }
...
export const deleteSalaryThunk = createAsyncThunk(
  ...
  async (salaryId, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.delete(API_ENDPOINTS.SALARY.DELETE(salaryId));
      ...
      dataCache.optimisticUpdate('salaries', (cachedSalaries) => {
        if (Array.isArray(cachedSalaries)) {
          return cachedSalaries.filter(salary => salary.id !== parseInt(salaryId));
        }
        return cachedSalaries;
      });
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to delete salary configuration';
      return rejectWithValue(message);
    }
  }
);
...
export const fetchSalariesByEmploymentTypeThunk = createAsyncThunk(
  ...
  async ({ employmentType, forceRefresh = false }, { rejectWithValue }) => {
    try {
      ...
      const filteredSalaries = allSalaries.filter(salary => salary.employmentType === employmentType);
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to fetch salaries by employment type';
      return rejectWithValue(message);
    }
  }
);
...
export const searchSalariesThunk = createAsyncThunk(
  ...
  async ({ query, employmentType, forceRefresh = false }, { rejectWithValue }) => {
    try {
      ...
      if (employmentType && employmentType !== 'all') {
        filteredSalaries = filteredSalaries.filter(salary => salary.employmentType === employmentType);
      }
      ...
      const message = error?.response?.data?.message || error.message || 'Failed to search salaries';
      return rejectWithValue(message);
    }
  }
);
  ...
  async (salaryIds, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (successful.length > 0) {
        ...
        dataCache.optimisticUpdate('salaries', (cachedSalaries) => {
          if (Array.isArray(cachedSalaries)) {
            return cachedSalaries.filter(salary => !successful.includes(salary.id));
          }
          return cachedSalaries;
        });
